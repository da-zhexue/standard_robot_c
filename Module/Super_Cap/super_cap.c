...
void get_supercap_power(const uint8_t* rx_data, void* arg);
void get_supercap_remain(const uint8_t* rx_data, void* arg);
void super_cap_init(super_cap_instance* supercap_ins, CAN_HandleTypeDef *hcan)
{
    ...
    BSP_CAN_RegisterCallback(supercap_ins->can_ins, SUPERCAP_RX_POWER_ID, CAN_ID_STD, get_supercap_power, &supercap_ins->power_data);
    BSP_CAN_RegisterCallback(supercap_ins->can_ins, SUPERCAP_RX_REMAIN_ID, CAN_ID_STD, get_supercap_remain, &supercap_ins->power_data);
}
